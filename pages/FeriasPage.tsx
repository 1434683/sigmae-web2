            ...
            <Modal isOpen={isModalOpen} onClose={() => setModalOpen(false)} title={currentFerias?.id ? 'Editar Férias' : 'Agendar Férias'}>
                {currentFerias && <form onSubmit={e => { e.preventDefault(); handleSave(); }} className="space-y-4">
                    <div className="relative" ref={searchContainerRef}>
                        ...
                        <input type="text" placeholder="Buscar..." value={policialSearch} onChange={e => {setPolicialSearch(e.target.value); if (currentFerias?.policialId) setCurrentFerias(p => ({ ...p, policialId: undefined })); setDropdownOpen(true);}} onFocus={() => { if (!currentFerias.id) setDropdownOpen(true); }} className="mt-1 w-full border rounded-md p-2" disabled={!!currentFerias.id} autoComplete="off"/>
                        {isDropdownOpen && !currentFerias.id && (<ul className="absolute z-10 w-full bg-white border mt-1 max-h-60 overflow-y-auto shadow-lg">{filteredPoliciaisParaSelecao.length > 0 ? filteredPoliciaisParaSelecao.map(p => (<li key={p.id} className="px-3 py-2 cursor-pointer hover:bg-pm-blue-light hover:text-white" onClick={() => {setCurrentFerias(prev => ({...prev, policialId: p.id })); setPolicialSearch(`${p.postoGrad} ${p.nome} (${p.re})`); setDropdownOpen(false);}}>{p.postoGrad} {p.nome}</li>)) : <li className="px-3 py-2 italic">Nenhum policial.</li>}</ul>)}
                    </div>
